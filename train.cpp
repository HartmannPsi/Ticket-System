...
TrSys::~TrSys() {
  ...
  /*
    ...
    for (auto it = stats.cbegin(); it != stats.cend();
      ...
      // }
      stat_file.seekp(i);
      stat_file.write(reinterpret_cast<const char *>(&stat),
    sizeof(Stat_serial));
    }*/
  /*
    if (max_serial > 48) {
...
bool TrSys::add_train(const std::string &id, int stat_num, int seat_num,
  ...
  for (int i = 0; i != stat_num; ++i) {
    Stat_serial ind(stations[i]);
    // auto it = serials.find(stations[i]);
    if (!serials.at(ind)) {

      ind.serial = max_serial;
      stats.insert(ind);
      serials.insert(ind);
...
std::string TrSys::query_train(const std::string &id,
  ...
  if (train_data.at(train)) {
    ...
    if (train.start_sale <= day && day <= train.end_sale) {
      ...
      // Stat_serial tmp_stat;
      // tmp_stat.serial = train.stations[0];
      // stats.at()
...
vector<std::string> BPlusTree<Station, 113>::find_trains(const Station &ind) {
  ...
  while (true) {
    ...
    if (i < node.size && ind.name < node.index[i].name) {
      break;
    }
...
std::string TrSys::query_ticket(const std::string &from, const std::string &to,
  ...
  // if (TIME == 2864227) {
  //   std::cout << query_train("THISMOMENTYEARNING", day);
  // }
  ...
  // for (int i = 0; i != from_train_ids.size(); ++i) {
  //   std::cout << from_train_ids[i] << '\n';
  // }
  ...
  for (int i = 0; i != train_ids.size(); ++i) {
    ...
    if (_from == train.stations[train.stat_num - 1]) {
      // if (TIME == 2864227 && train_ids[i] == "THISMOMENTYEARNING") {
      //   std::cout << "TAG_1\n";
      // }
      continue;
    }
    ...
    if (j == train.stat_num || train.stations[j] != _to) {
      // if (TIME == 2864227 && train_ids[i] == "THISMOMENTYEARNING") {
      //   std::cout << "TAG_3\n";
      // }
      continue;
...
int TrSys::buy_ticket(const std::string &usr, const std::string &id, int day,
  ...
  if (train_data.at(train) && train.released) {
    ...
    if (j == train.stat_num || train.stations[j] != _from) {
      return -1;
    }
